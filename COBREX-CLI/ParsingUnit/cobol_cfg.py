...
class CFGBuilder(Cobol85Visitor):
    ...
    def visitEvaluateStatement(self, ctx:Cobol85Parser.EvaluateStatementContext):
        ...
        evaluate_block = self.current_block
        ...
        for child in ctx.getChildren():
            if isinstance(child,Cobol85Parser.EvaluateWhenPhraseContext):
                ...
                self.add_exit(evaluate_block,next_when_block,"evaluate when")
                ...
                # self.current_block = next_when_block
                self.add_exit(evaluate_block, other_scope, "false")
    ...
    def visitIfStatement(self, ctx:Cobol85Parser.IfStatementContext):
        ...
        if if_else_ctx is not None:
            ...
            self.add_exit(self.current_block, after_if_block, "outside-if")
        ...
        end_if = Statement('END-IF',"end-if")
        self.add_statement(after_if_block,end_if)
        self.statements.append(end_if)
        self.current_block = self.new_block()
        # self.current_block.predecessors.append(after_if_block)
        self.add_exit(after_if_block,self.current_block)
